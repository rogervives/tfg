...
def fes_gravacio():
    ...
    def dire():
        ...
        #directorio_actual.set(r"C:\Users\roger\OneDrive\UNi\Unidara\TFG\PlanA\Repositori\shazamio\MAIN\Recordings")
    ...
    def abrir():
        ...
        open_archive = filedialog.askopenfilename(initialdir="/",
                                                  title="Seleccionar archivo", filetypes=(("mp3 files", "*.mp3"),
                                                                                         ("all files", "*.*")))
        if open_archive != "":
            try:
                reproduciendo = True
    ...
    def grabacion(FORMAT, CHANNELS, RATE, CHUNK, audio, archivo):
        ...
        for i in grabs:
            if "gravacion" in i:
                count += 1
        if count > 0:
            archivo = "gravacion" + "(" + str(count) + ")" + ".mp3"
    ...
    btnAbrir = Button(ventana, text="Obrir", width=16, command=abrir)
